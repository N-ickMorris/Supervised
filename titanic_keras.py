...
if classifier:
    ...
    model = build_nnet(features=X.shape[1], targets=Y.shape[1], layer=[64, 64],
                       learning_rate=0.001, l1_penalty=1e-3, classifier=True)
else:
    class_weights = None
    model = build_nnet(features=X.shape[1], targets=Y.shape[1], layer=[64, 64],
                       learning_rate=0.001, l1_penalty=1e-3, classifier=False)

# train the model
model.fit(X.iloc[train_idx, :], Y.iloc[train_idx, :], epochs=50, batch_size=16, class_weight=class_weights)
