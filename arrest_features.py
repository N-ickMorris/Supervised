...
# separate X and Y
X = data.drop(columns=["released_Yes", "released_No"]).copy()
Y = data.drop(columns=X.columns).drop(columns=["released_No"]).copy()
...
# export the data
X.to_csv("X arrest.csv", index=False)
Y.to_csv("Y arrest.csv", index=False)
